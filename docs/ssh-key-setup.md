...
2. 希望名 `taro`、公開鍵の1行全体、公開鍵のFingerPrint（指紋）を申請する。
...
## 4. 公開鍵とFingerPrint（指紋）を提出する

FingerPrintは鍵を照合するための短い識別値で、人の指の指紋ではありません。このガイドでは `SHA256:...` の形式を使います。申請時に提出するのは**本人の公開鍵のFingerPrint**、初回SSH接続時に照合するのは**接続先サーバーのホスト鍵のFingerPrint**です。
...
最初のコマンドで表示される `ssh-ed25519 AAAA... コメント` の**1行全体**を提出します。画面上で折り返されていても、途中に改行や空白を挿入しません。2つ目の出力にある `SHA256:...` がFingerPrint（指紋）です。`AAAA...` や `SHA256:...` は説明用の省略表記であり、実際の申請には自分の出力を省略せず使用します。
...
管理者から確定したFreeIPAユーザー名、公開鍵登録完了とFingerPrint（指紋）、許可GPU、サーバーのホスト鍵のFingerPrint（指紋）を受け取ります。`-campus` では踏み台の `campus-relay` への本人の公開鍵登録完了と中継利用許可も必要です。**鍵を作っただけ、または申請しただけでは接続できません。**
...
| 初回接続時の `SHA256:...` | サーバーのホスト鍵のFingerPrint（指紋）。提出した自分の公開鍵のFingerPrint（指紋）とは別。管理者から通知された接続先のFingerPrint（指紋）と照合する |
| `Permission denied (publickey)` | 確定ユーザー名、`IdentityFile`、登録された公開鍵のFingerPrint（指紋）を確認し、拒否された接続先とエラーを管理者へ連絡する |
